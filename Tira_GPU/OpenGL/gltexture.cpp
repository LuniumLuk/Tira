...
auto GLTexture::bind(unsigned int binding) -> void {
...
auto GLTexture::bind(ShaderProgram* shader, std::string const& name, unsigned int binding) -> void {
    glActiveTexture(GL_TEXTURE0 + binding);
    switch (type) {
    case TextureType::Texture2D:
        glBindTexture(GL_TEXTURE_2D, handle); break;
...
GLTexture2D::GLTexture2D(std::string const& filename, bool hdr, bool genMipmap, bool flipY) {
...
GLTexture2D::GLTexture2D(size_t width, size_t height, InternalFormat format, void* data) {
    type = TextureType::Texture2D;
    glBindTexture(GL_TEXTURE_2D, handle);
...
GLCubemap::GLCubemap(std::vector<std::string> const& filenames, bool genMipmap, bool flipY) {
